...
import kotlinx.android.synthetic.main.activity_form_reconexion.*
import javax.inject.Inject

class FormReconexionActivity : DaggerAppCompatActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_form_reconexion)
    ...
    private fun getLecturaByOrden(number: Int) {
        suministroViewModel.getSuministroReconexionByOrdenTask(number)
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe(object : Observer<SuministroReconexion> {
                override fun onComplete() {}
                override fun onSubscribe(d: Disposable) {}
                override fun onError(e: Throwable) {}
                override fun onNext(s: SuministroReconexion) {
//                    clearText()
                    showSuministro(s)
                }
            })
    ...
    private fun showSuministro(it: SuministroReconexion) {
        ...
        textViewConexion.text = String.format("Primera Reconexión : %s", it.primeraReconexion)
    ...
    private fun formRegistro() {
        ...
        if (gps.isLocationEnabled()) {
            if (gps.getLatitude().toString() == "0.0" || gps.getLongitude().toString() == "0.0") {
                ...
                if (estado == 4) {
                    if (r.parentId == 93) {
                        if (editTextArtefacto.text.toString().isEmpty()) {
                            Util.toastMensaje(this, "Seleccione artefacto")
                            return
                        }
                    }
                }
